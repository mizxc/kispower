...
; (function ($w) {
    ...
    // backgroundColor
    var __backgroundColor__ = '#ffffff';
    // lineColor
    var __lineColor__ = '#555';
    // displayMode
    var __displayMode__ = 'side';
    // saveTime
    var __saveTime__ = 'saveTime';
        ...
        add_node: function (parent_node, nodeid, topic, data, idx, direction, expanded) {
            ...
            if (parent_node.isroot) {
                ...
                if(this.displayMode == 'full'){
                    ...
                    node = new jm.node(nodeid, nodeindex, topic, data, false, parent_node, 1, expanded);
                }
            } else {
        ...
        node_tree: {
            example: {
                "meta": {
                    ...
                    "lineColor": __lineColor__,
                    "displayMode":__displayMode__,
                    "saveTime":__saveTime__,
                },
                "format": "node_tree",
            ...
            get_mind: function (source) {
                ...
                mind.backgroundColor = source.meta.backgroundColor;
                mind.lineColor = source.meta.lineColor;
                mind.displayMode = source.meta.displayMode;
                mind.saveTime = source.meta.saveTime;
            ...
            get_data: function (mind) {
                ...
                json.meta = {
                    ...
                    backgroundColor:mind.backgroundColor,
                    lineColor:mind.lineColor,
                    displayMode:mind.displayMode,
                    saveTime:mind.saveTime
                };
    ...
    jm.prototype = {
        ...
        dblclick_handle: function (e) {
            ...
            if (this.get_editable()) {
                ...
                var nodeid = this.view.get_binded_nodeid(element);
                if(!nodeid){
                    nodeid = $($(e.target).parents('jmnode')[0]).attr('nodeid');
                }
                if(nodeid){
                    this.select_node(nodeid)
                    jm_editNode();
        ...
        get_meta: function () {
            return {
                ...
                backgroundColor:this.mind.backgroundColor,
                lineColor:this.mind.lineColor,
                displayMode:this.mind.displayMode,
                saveTime:this.mind.saveTime
            };
        },
        ...
        //addbyxxj
        get_nodeData: function (node) {
            return this.data.get_nodeData(node);
        },
    ...
    jm.data_provider.prototype = {
        ...
        //addbyxxj
        get_nodeData: function (node) {
            return jm.format.node_tree.get_nodeData(node);
        }
    };
    ...
    jm.view_provider.prototype = {
        ...
        edit_node_begin: function (node) {
            ...
            if(parseInt(this.e_editor.style.width)<200){
                this.e_editor.style.width = "200px";
            }
            element.innerHTML = '';
    ...
    jm.shortcut_provider.prototype = {
        ...
        handler: function (e) {
            // 如果模态窗口打开，屏蔽快捷键
            if($("#edit-title-Modal").hasClass('show')){return}
            if($("#share-node-Modal").hasClass('show')){return}
            if($("#export-node-Modal").hasClass('show')){return}
            if($("#import-node-Modal").hasClass('show')){return}
            //if(e.target.tagName=='INPUT' || e.target.className=='simditor-body'){ return }
        ...
        handle_addchild: function (_jm, e) {
            ...
            if (!!selected_node) {
                ...
                var nodeColor = jm_getNodeColor('child')
                var node = _jm.add_node(selected_node, nodeid, 'New Node', {'background-color':nodeColor[0],'foreground-color':nodeColor[1],'topicType':'ordinary'});
                //var node = _jm.add_node(selected_node, nodeid, 'New Node', {'topicType':'ordinary'});
                if (!!node) {
        ...
        handle_addbrother: function (_jm, e) {
            ...
            if (!!selected_node && !selected_node.isroot) {
                var nodeid = jm.util.uuid.newid();
                var nodeColor = jm_getNodeColor('brother')
                var node = _jm.insert_node_after(selected_node, nodeid, 'New Node', {'background-color':nodeColor[0],'foreground-color':nodeColor[1],'topicType':'ordinary'});
                //var node = _jm.insert_node_after(selected_node, nodeid, 'New Node', {'topicType':'ordinary'});
                if (!!node) {
                    _jm.select_node(nodeid);
                    //_jm.begin_edit(nodeid);
                    jm_editNode()
